...
.. deprecated::
    The standalone ``main()`` entry is DEPRECATED.
    Use the unified feedback loop pipeline instead:
      1. ``run_and_collect.py``  — generate + execute + collect logs
      2. ``processor.py``        — process logs, generate constraints, produce pending_review.md
      3. Human review            — review pending_review.md, mark approved items
      4. Re-ingest               — approved items → knowledge_base JSONs → rag_engine.py

    This module's ``_heuristic_constraints()`` and ``map_events_to_api()`` are still
    imported and used by ``processor.py``; only the standalone ``main()`` is deprecated.

Pipeline (adapted for feedback_loop):
...
# 确保能 import rag_demo 根目录和 feedback_loop/loop_c 下的模块
...
def _heuristic_constraints(events: List[MappedEvent]) -> List[str]:
    ...
    for e in events:
        ...
        if m7:
            ...
            continue
        # 无法泛化为 API 使用约束（如 CUDA/Taichi 内部错误、OOM 等），跳过
        continue
    seen = set()
    out = []
    for c in cons:
...
def main():
    import warnings
    warnings.warn(
        "constraint_builder.py standalone main() is DEPRECATED. "
        "Use run_and_collect.py + processor.py instead. "
        "See feedback_loop/问题诊断与回应汇总.md P2.5.",
        DeprecationWarning,
        stacklevel=2,
    )
    print("[DEPRECATED] 此入口已废弃。请使用: python run_and_collect.py + python processor.py")
    ...
    if args.run_agent:
        ...
        from agent import GenesisAgent  # rag_demo root
